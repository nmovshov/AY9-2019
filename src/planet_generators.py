...
def tripquad_reftop(N, x, ref):
    """Return piecewise-quadratic density profile stitched to reference density.

    tripquad_reftop(N, x, ref) returns an (N+n)-point density profile, i.e. a
    (zvec,dvec) tuple, where the density D is approximated by a
    piecewise-quadratic function in the normalized radius Z except for the
    uppermost n levels which instead follow the reference density structure given
    by ref. The n-by-2 array ref defines the density, ref[:,1], on the level radii
    ref[:,0]. For the rest of the planet, each quadratic segment is defined by its
    ...
    left-limit). The density at z=0 is d33. The uppermost segment passes through
    the point (z,d)=ref[-1,:]. This leaves 10 free parameters. They are ordered
    as follows:

        x = [a1, y11, a2, y21, y22, a3, y32, y33, lz1, lz2]
    where
...
"""
    ...
    s0 = ref[0,0]
    zref = ref[:,0]/s0
    zvec = np.linspace(zref[-1] - 1/N, 1/N, N)
    svec = zvec*s0

    # Interpreting the parameters
    a1 = x[0]; y11 = x[1]
    a2 = x[2]; y21 = x[3]; y22 = x[4]
    a3 = x[5]; y32 = x[6]; y33 = x[7]
    lz1 = x[8]; lz2 = x[9]
    ...
    # Return
    svec = np.hstack((ref[:,0], svec))
    dvec = np.hstack((ref[:,1], dvec))
    return (svec, dvec)
...
def pppp_quartop(N, x, z0, R_m, quart):
    """Preferred Piecewise Polynomial Parameterization, with a quartic top.

    pppp_quartop(N, x, z0, R_m, quart) returns an N-point density profile,
    i.e. a (zvec,dvec) tuple, where the density D is approximated by a
    piecewise-quadratic function in the normalized radius Z=r/R_m up to Z=z0
    and follows a quartic polynomial above z0. The array quart holds quartic
    coefficients in descending powers. Inside of z0, each quadratic segment is
    ...
    are at z1 (upper) and z2 (lower). The density at z0 is d10. The density at z1
    is d11 (the right-limit) and d21 the left-limit). The density at z2 is d22
    (the right-limit) and d32 (the left-limit). The density at z=0 is d33. The
    curvature of the innermost segment is assumed to be zero (i.e. a linear core).
    The 10 remaining free parameters are ordered as follows:

        x = [a1, y10, y11, a2, y21, y22, <a3=0>, y32, y33, lz1, lz2]
    where
        a1: curvature of first segment (a1*x^2 + b1*x + c1)
        y10: d10
        y11: d11 - d10
        ...
        y21: d21 - d11
        y22: d22 - d21
        <a3 would be here but it's always zero>
        y32: d32 - d22
        y33: d33 - d32
        lz1: z1
        lz2: z2
